...
public class ModbusTCPListener implements Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(Modbus.LOGGER_NAME);
	...
	public void run() {
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("Listenening to " + m_ServerSocket.toString() + "(Port "
						+ m_Port + ")");
			}
			...
			do {
				...
				if (m_Listening) {
					...
					m_ThreadPool.execute(new TCPConnectionHandler(new TCPSlaveConnection(
							incoming)));
					count();
				} else {
